...
//	zmin_cor = (realtype *) malloc(DS->NumEle * sizeof(realtype));
	...
	DS->Total_water_out = (realtype *)malloc(DS->NumEle*sizeof(realtype));
	DS->Total_water_in = (realtype *)malloc(DS->NumEle*sizeof(realtype));
	#ifdef LE_PIHM
	...
	for (i = 0; i < DS->NumEle; i++) {
		...
		//? ? XUAN FOR SHALEHILLS ONLY
			...
			DS->Ele[i].RhoBed = DS->LE_bedrock[(DS->Ele[i].bedrock - 1)].RhoBed;
			DS->Ele[i].Diameter = DS->LE_soil[(DS->Ele[i].soil - 1)].Diameter;
			DS->Ele[i].Uplift = DS->LE_bedrock[(DS->Ele[i].bedrock - 1)].Uplift; /*Bedrock uplift rate*/
			DS->Ele[i].CoefP0 = CS->Cal.CoefP0*DS->LE_bedrock[(DS->Ele[i].bedrock - 1)].CoefP0; /*Fitting constant in weather equation*/
			DS->Ele[i].DReg = CS->Cal.DReg*DS->LE_soil[(DS->Ele[i].soil - 1)].DReg;
			DS->Ele[i].P0 = CS->Cal.DReg*DS->LE_bedrock[(DS->Ele[i].bedrock - 1)].P0; /*Regolith production rate in the absence of soil above bedrock*/
		#endif
		...
		if (newelev_file == NULL) {
			...
			for (i = 0; i < DS->NumEle; i++) {
				...
				fscanf(newelev_file, "%lf %lf",&tempvalue4, &tempvalue5);
					...
					if (CS->init_type == 0)
					{
						NV_Ith_S(CV_Y, i + 2 * DS->NumEle) = (tempvalue4 - tempvalue5)-0.1;
					}
				#else
